...
for nb in range(0,72001):
    n += 1
    req = requests.get(apiMaison, headers=entetes) ### ATTENTION, ICI TU TE CONNECTES À MON SITE 72000 FOIS! :)
    obj = req.json()
    #Avec les deux lignes suivantes j'ajoute les valeurs n et nb à ma liste
    info.append(n)
    info.append(nb) ### N ET NB ONT DES VALEURS QUASI-ÉQUIVALENTES
    print(info)
    ...
    if "limat" in obj["registre"][0][1][0]["objet"]:
        ### POUR LA SOLUTION ÉLABORÉE QUE TU AS TENTÉE ICI, JE VAIS TE DONNER UN A-, MÊME SI JE RÉSERVE NORMALEMENT LES A POUR DES SCRIPTS QUI PRODUISENT CE QUI ÉTAIT DEMANDÉ
       l1 = {
